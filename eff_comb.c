...
void GetEff(std::string inputFile1, std::string inputFile2, double pTBin[], int size, bool muonTrig, double y1, double y2, const char* title, const char* outPath1, const char* outPath2, const char* outPath3, const char* outPath4)
{
  ...
  { 
    ...
    for(uint iReco=0; iReco<tree1.candSize(); iReco++){
      ...
      const auto& pD1 = tree1.pTD1()[iReco]*std::cosh(tree1.EtaD1()[iReco]);
      const auto& pD2 = tree1.pTD2()[iReco]*std::cosh(tree1.EtaD2()[iReco]);
      const bool inMuonAcceptance = (pD1 > 3.5 && fabs(tree1.EtaD1()[iReco])<2.4) && (pD2 > 3.5 && fabs(tree1.EtaD2()[iReco])<2.4);
  ...
  for(Long64_t jentry=0; jentry<nevents2; jentry++)
  {
    ...
    for(uint iReco=0; iReco<tree2.candSize(); iReco++){
      ...
      const auto& pD1 = tree2.pTD1()[iReco]*std::cosh(-tree2.EtaD1()[iReco]);
      const auto& pD2 = tree2.pTD2()[iReco]*std::cosh(-tree2.EtaD2()[iReco]);
      const bool inMuonAcceptance = (pD1 > 3.5 && fabs(-tree2.EtaD1()[iReco])<2.4) && (pD2 > 3.5 && fabs(-tree2.EtaD2()[iReco])<2.4);
      if (!inMuonAcceptance) continue;
